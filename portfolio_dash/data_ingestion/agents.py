...
# The local-exchange-code SHAPE each market's symbols take. Deliberately loose (this is a
# soft hint, never a gate): TW = TWSE/TPEx numeric code incl. 5–6-digit ETF/TDR codes and
# preferred/bond suffixes (2330, 00878, 00679B, 2881A); US = a ticker starting with a letter
# (AAPL, BRK.B); MY = 4-digit Bursa code + optional suffix (1155, 0800EA), ``.KL`` tolerated.
# A mismatch (the owner's bug: 聯電 parsed to the US ADR "UMC" on a tw_broker row) appends a
# needs_confirm WARNING issue to the row — it never blocks and never rewrites the symbol;
# the REAL provider lookup at registration remains the authority.
_SYMBOL_FORMAT: dict[Market, re.Pattern[str]] = {
    Market.TW: re.compile(r"^\d{4,6}[A-Z]{0,2}$"),
    Market.US: re.compile(r"^[A-Z][A-Z0-9.\-]{0,9}$"),
    Market.MY: re.compile(r"^\d{4}[A-Z0-9]{0,4}(\.KL)?$"),
}
...
def _append_format_warning(
    ...
    resolved payload symbol (falling back to the draft's), so a CJK name that
    fuzzy-resolved to a proper local code is never false-flagged. Skipped when the row
    already carries the HARD ``market_mismatch`` coherence issue (no double flag), when
    the account is unknown, or when the symbol is blank (other issues cover those)."""
    ...
    if not sym or _SYMBOL_FORMAT[market].match(sym):
        return
    row.issues.append(
        Issue(
