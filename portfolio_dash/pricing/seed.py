...
def pending_seed_removal(
    ...
    "does another corporate action that stays in the ledger create this same child on this
    same day?" — then the seed still belongs to that one and stays.
    ...
    try:
        row = conn.execute(
            "SELECT source, fetched_at FROM prices WHERE instrument=? AND as_of_date=?",
            (symbol, on.isoformat()),
        ).fetchone()
    ...
    return SeedPriceVerdict(symbol=symbol, as_of=on, removable=True)
