...
class NewsPromptWire(TypedDict):
    ...
    badge has one truth and the frontend never holds the official body.
    """
...
def _wire(body: str, updated_at: str) -> NewsPromptWire:
    return {
        "body": body,
...
def get_news_prompt(conn: sqlite3.Connection) -> NewsPromptWire:
    """The news-organizer prompt wire (default-safe): body, stamp, official version, and
    whether the stored body still IS the official one."""
    ensure_news_prompt_seeded(conn)
    row = conn.execute(
        "SELECT body, updated_at FROM news_prompt_config WHERE id = 1"
    ).fetchone()
    if row is None:
        return _wire(official_templates.NEWS_ORGANIZER_PROMPT, _SEED_AT.isoformat())
    return _wire(str(row["body"]), str(row["updated_at"]))


def set_news_prompt(conn: sqlite3.Connection, body: str, *, now: datetime) -> NewsPromptWire:
