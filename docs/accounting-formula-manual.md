...
種子費率見 `config_seed.py::FEE_RULES`；owner 完整費表已在案（→ §3.5、`docs/reference/broker-fee-schedules-2026-07.md`），fee-engine-v2 升級後將取代種子費率，**升級前以現行引擎所計為裁定準據**。
...
### 3.5 覆寫（overrides）與費率規制（owner 完整費表已在案）

- **手動覆寫**：使用者於輸入／編輯時可顯式改寫 `fee` / `tax`；此時系統以覆寫值為準，並在 `snapshot`
  標記 `override: true`（見 §10 之 `_recompute_edit_fees`）。
- **owner 完整費表已在案，於 fee-engine-v2 升級時取代現行種子費率（誠實聲明）**：owner 已於 2026-07-15
  提供三家券商之**完整真實費表**（→ `docs/reference/broker-fee-schedules-2026-07.md`）。該費表為獨立之
  「fee engine v2」工作包之來源規格，**升級後將取代**目前 `config_seed.py::FEE_RULES` 之種子費率。在
  fee-engine-v2 出貨前，**本 §3 記述的是「目前引擎實際計算」之數字**——此正是裁定「站上目前金額」所需之
  依據（仲裁對象永遠是 app 當下實際產生之數字，而非尚未實作之未來費表）。
- **已知分歧（現行引擎 vs owner 完整費表；升級前一律以現行引擎為裁定準據）**：
  - **US `sec_fee`**：現行 `0.0000278`（買賣同式套用）；費表為 **`0.0000206`、僅賣方、最低 `$0.01`**。
  - **US 其他規費尚未建模**：**TAF**（`$0.000195/股`，賣方，最低 $0.01、最高 $9.79）、**CAT**、**平台費**
    （Moomoo 整股 `$0.99`／Schwab `$0`）、**交收費（settlement）**（`$0.003/股`，上限 notional×1%）皆尚未進
    現行引擎。
  - **MY 費表結構不同**：費表為 **佣金 `0.03%`（最低 RM0.01）+ 平台費 RM3 + SST（＝佣金＋平台費＋清算費
    合計之 8%）+ 印花 `ceil(金額/1000)×RM1`（各證券類型分別設上限、ETF 免徵）**；現行引擎為 佣金 `0.08%`
    （最低 RM3）+ 清算 `0.03%`（上限 RM1,000）+ SST 0，印花 `0.1%` 無上限（見 §3.4）——**費率、最低值、
    上限結構、SST 有無均不同**。
  - **TW 群益 2.3 折「先收後退」模型**：交割當下（T+2）收足 `0.1425%` 原價，次月退回 77%（實收 2.3 折）。
    現行引擎僅一次性收 `0.1425%×discount`，**未建模「先收後退」之退佣分錄**——退佣帳本處理**待 owner
    設計裁定**。
  - **TW 費用捨入分歧**：群益範例對手續費採**無條件捨去**（`142.5→142`）；現行引擎為 `ROUND_HALF_UP`
    （`round_integer=True`，見 §3.1，`142.5→143`）——**捨入方向待 owner 裁定**。
- **費率變更之流程**：升級後費率變更屬 config 變更，須記於 `CHANGELOG.md`；每筆交易之 `fee_rule_snapshot`
  逐筆保存當時所用費率，故新舊費制並存期間，**每列以其快照所屬費制裁定**（見 §12.4 費用爭議註記）。

> **實作位置**：`data_ingestion/fees.py`、`data_ingestion/config_seed.py::FEE_RULES`；完整費表
> `docs/reference/broker-fee-schedules-2026-07.md`（fee-engine-v2 來源規格）。
> **依據**：`.claude/rules/markets-and-fees.md`。
> **驗證錨點**：上表各 `fee_engine.*`（壓測 phase1，`fee_engine.fee`／`fee_engine.tax` 各 36 項全通過）——
> 錨定的是**現行引擎**之輸出（非上述 owner 費表）。
...
> **費用爭議專屬註記（現行引擎 vs owner 完整費表）**：裁定任一費／稅金額時，須**同時**查對 §3（現行引擎所計）
> 與 `docs/reference/broker-fee-schedules-2026-07.md`（owner 完整費表 / fee-engine-v2 來源規格），並確認該爭議
> 列**係以何費制入帳**——**逐筆 `fee_rule_snapshot`（§3、§10.2）為最終裁定依據**：快照記錄當筆實際套用之費率，
> 故 fee-engine-v2 出貨後，舊列仍以其快照之舊費制裁定、新列以新費制裁定，兩者並行不悖。
