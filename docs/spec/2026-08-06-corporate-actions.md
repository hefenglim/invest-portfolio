...
**Status:** Owner decisions **D1–D48 approved** (§8; D44 / D47 / D48 on 2026-08-15, D45 and
D46 on 2026-08-11/12; D30–D39 on 2026-08-10, from the
...
| 2026-08-12 | **Two owner rulings the revision log never received (added 2026-08-15).** **D45 — D36's whole-account IRR is RETIRED**, unimplemented: 「IRR 標記不需要」. D36 is struck in §8 and **D12's blind spot returns to standing** — the reorganisation fee is invisible to every return metric this system has. §3.3, §7.5 and both manual mirrors now say so plainly rather than promising a fix, because a manual promising a fix that never arrives stops the reader looking for the workaround. **D46 — §10.5's acceptance run uses the SIMULATED corpus**; the real-statement run is reclassified from release gate to post-release verification, which is the owner's scope call to make and this row exists so nobody later mistakes it for having been done |
| 2026-08-15 | **The last open decision closed, and two more the owner gave with it.** **D44 — option (b)**: a SPLIT dated after the owner's target band was set raises a soft finding **quoting the restated value**, and the form offers it as a checkbox that **defaults to off** (doing nothing keeps what they typed — (c) retained as the floor). Two amendments make it work: the finding **carries its repair** (E23's shape — a soft `needs_confirm` alone would block, then commit, and leave the stale band and the fix on another page), and a new nullable **`instruments.target_set_at`** makes 「predates」 answerable — without it the finding fires on every split of a banded symbol, so a bulk broker import of years of history is mostly false positives, which is the cry-wolf failure E23 spent a fourth condition avoiding. **D47 — an EXCHANGE MOVES the band to the new ticker, values unchanged** (a copy would leave a dead ticker alerting forever; `target_cross` fires on registration, not on holding). **D48 — a SPINOFF auto-registers its child**, inheriting the parent's market and currency (E11 requires them to match, so it is a derivation), **soft rather than silent** so a mistyped CUSIP never becomes a phantom instrument — narrowed to SPINOFF, with sources and EXCHANGE destinations keeping E10's hard rejection — **and the form can seed the child's first price**, because one unpriced holding blanks the WHOLE portfolio's XIRR and a spin-off guarantees one |
...
> ~~**The limitation is no longer permanent (D36, 2026-08-10).**~~ **SUPERSEDED 2026-08-11 by D45 —
> the owner retired D36.** D36 had planned a **whole-account IRR** alongside XIRR, which *would* have
> seen a `WITHDRAW`, and on that basis this note said D12's blind spot was temporary and instructed
> §7.5/W9 to document it as such. D36 was never implemented and the owner has now ruled it not
> needed, so **the limitation stands, and it is permanent until some future metric changes it.**
>
> The correction matters more than the wording: the manual is the accounting **authority**, and a
> paragraph promising a resolution that will never arrive is worse than one stating a blind spot
> plainly — the reader stops looking for the workaround. The reorganisation fee is booked as a
> `WITHDRAW` cash movement and is **invisible to every return metric this system has**. XIRR stays
> deliberately untouched (every historical figure, manual anchor and oracle expectation holds), and
> the fee is visible where cash movements are visible: the cash ledger and net worth.
...
- ⚠ **The owner-entered target BAND is deliberately not re-expressed** — it is a configured intent,
  ...
  28.57 or as 200. It is compared against a re-expressed price, so a stale band crosses on the
  split date and stays crossed. **D44 (owner ruling 2026-08-15) resolves this at ENTRY, not here:**
  recording a SPLIT raises the soft `target_band_predates_split`, quoting the restated value, and
  the form offers it as an opt-in that **defaults to off**. The read path above is unchanged — the
  band the rule compares is whatever the owner last decided it should be.
...
**Two limitations must be written up. This paragraph has now been rewritten twice — read the whole
of it before editing the manual, because its first two versions each documented a decision that had
already moved.** **D11** (volume is not un-adjusted) is a standing limitation and always was.
**D12** is now **also** a standing limitation: D36 would have resolved it with a whole-account IRR,
and **D45 (owner, 2026-08-11) retired D36 unimplemented**. The manual must therefore say plainly
that the reorganisation fee is booked as a `WITHDRAW` and is **invisible to every return metric
this system has** — XIRR deliberately so, which is why every historical figure, worked anchor and
oracle expectation stays put — and that it is visible only where cash movements are: the cash
ledger and net worth. The text must **not** say "pending D36", and must not imply a resolution is
coming; a promised fix that never arrives stops the reader looking for the workaround, which is
strictly worse than a blind spot stated once. Cash-and-stock mergers are
...
(the amount-fuzzed acceptance corpus) are the owner's own rulings. **D44 was the last open decision and was ruled on 2026-08-15** (option (b), with two amendments
stated in its row), in the same sitting as **D47** and **D48** — two rulings the owner gave on the
same question asked of the other two action kinds, and recorded as their own rows because a
decision is one row (§8's own convention). **Nothing is open.**
...
| ~~**D36**~~ *(blocks W9's wording only)* | ~~Does anything besides trades reach the return metrics?~~ | **SUPERSEDED 2026-08-11 (D45 — owner retired it, unimplemented).** The original outcome is kept below because W9's manual text was written against it and the correction is only legible beside it. Verified in place 2026-08-11: `portfolio/twr.py` holds `twr_index` / `convert_closes` / `build_overlay` and nothing named `account_irr`, so D36 never left the decision table. Consequence: **D12's blind spot is standing again** — the reorganisation fee is invisible to every return metric this system has, and §3.3 / §7.5 / the manual now say so without promising a resolution. — *original outcome:* **Option 2 — leave XIRR untouched, add a whole-account IRR in the existing `portfolio/twr.py`.** *System fit:* `twr.py` already exists as the home for a whole-account view; redefining XIRR would move every historical figure, invalidate the accounting manual's worked anchors and the stress-audit oracle's expectations, and require re-verification against the whole corpus — for amounts the backlog itself calls trivial. Additive instead. **It also resolves D12's conceded blind spot**: the reorganisation fee is booked as a `WITHDRAW`, which XIRR does not see and the account IRR does. §3.3, §7.5 and W9 state the limitation as *resolved*, not permanent |
| **D45** *(owner ruling 2026-08-11)* | **D36's whole-account IRR — build it, or retire it?** Approved 2026-08-10 from the spec-conflict audit, never implemented, and its only load-bearing consequence was the wording of one manual limitation. | **Retire it.** 「IRR 標記不需要」. D36 is struck above and **D12 returns to a standing limitation**. The propagation is the point: §3.3's note, §7.5's manual instruction and both manual mirrors each said the blind spot was temporary *because D36 was coming*, and a manual that promises a fix which never arrives is worse than one that states the blind spot plainly — the reader stops looking for the workaround. All of them now state it as standing. XIRR is untouched either way, so no historical figure, worked anchor or oracle expectation moves |
| **D46** *(owner ruling 2026-08-12)* | **§10.5's acceptance run was waiting on D37 — the owner's real per-symbol opening cost totals. Wait for them, or accept the simulated corpus?** This was the last thing blocking the release: D37's engineering half (the hard `original_cost_total > 0` validation) shipped days ago, and what remained was owner data with a long lead. | **Run it on the SIMULATED corpus; the real-statement run is not a release gate.** 「D37不用等我的對賬單資料，測試請使用模擬資料，進行造出符合需要測試的交易資料進行測試」. The D43 corpus already exercises D37's own case — four opening-inventory rows, two of them on a pre-action share basis — and the run PASSes on it while failing on a corpus one action row short, so the gate has been red and is therefore a gate. **The limit is recorded, not glossed:** a fuzzed corpus proves the feature accepts the *shape* of the owner's ledger, never that the owner's *actual* cost basis reconciles. Those are different claims and only the first is now evidenced. The real-data run remains available and remains the owner's to perform — it is reclassified from a release gate to a post-release verification, which is a scope decision the owner is entitled to make and which this row exists so nobody later mistakes for having been done |
...
| **D44** *(raised by W6c's integration review 2026-08-11; **owner ruling 2026-08-15**)* | **The owner-entered target BAND (`instruments.target_low` / `target_high`) is not re-expressed across a SPLIT.** W6c re-expresses the *price* that `target_cross` compares against, so after a 7-for-1 a stale band of 200 meets a price of ~28.6 and the rule crosses **immediately and permanently**. The defect pre-dates W6c — once a genuine post-split close was stored, the raw comparison did the same — but W6c makes it fire from the split date rather than from the next refresh, and this release is the first that knows a split happened. | **(b) — ask once, at entry, and show the restated number.** The three options were **(a) re-express the band**, **(b) flag at entry**, **(c) leave it, documented**. *(a) was rejected on this codebase's own terms, not on taste:* the band has **no `*_raw` column**, so an in-place rewrite is precisely the irreversible restatement **D39b** already refused for `open`/`high`/`low` — and doing it properly would mean `target_*_raw` + `target_basis` + a reconcile hook on every SPLIT insert/edit/delete, i.e. the whole W6a/W6b apparatus for a *configuration value*, which still would not resolve the ambiguity: 「alert me at 200」 may survive a 7-for-1 as 28.57 (a view about the company) or as 200 (a view about the share price). The spec's own discriminant settles it — every value that is a **machine-observed market fact** (`close`, `price_at_create`, the DRIP reinvest price) is re-expressed; every value the **owner typed** is not. *(c) was rejected because* the false cross is **permanent** and sits on the notification path, and `validate.py`'s E23 comment already names the cost: a guard that mostly cries wolf trains the owner to click through. **Two amendments make (b) work here.** **(1) The finding carries its repair** — E23's own shape (`identifier_change_repair` + `_split_conversion`), because a soft `needs_confirm` means *block, then commit*: acknowledged, the band is still stale, the false alert starts, and the fix lives on another page. So `restated_band` computes the value, the message quotes it (all three doors), and the form offers it as a checkbox that **defaults to OFF** — doing nothing keeps what the owner typed, which is (c) retained as the floor. **(2) A new nullable `instruments.target_set_at`**, without which "whose band *predates* it" is unanswerable and the finding must fire on **every** split of a banded symbol — so importing years of broker history, mostly historical splits against a band set last week, is mostly false positives. It is doing E23's fourth-term job. `NULL` = unknown = silent, the degradation `covered_ratio` and `_has_prices` already use. Owned end-to-end by `store.upsert_instrument`, which derives it by comparing against the stored row, so an unrelated edit cannot re-date the band |
| **D47** *(owner ruling 2026-08-15)* | **Does an EXCHANGE do anything to the owner's target band?** D44 answers the SPLIT case; an EXCHANGE re-keys the position onto a new symbol and the band stays behind on the old one. | **Move it to the new ticker, values unchanged.** 「ticker更換簡單一些可以直接換名字就好，其他都不動」. *A MOVE, not a copy, and that is the load-bearing half:* `target_cross` fires for every **registered** symbol carrying a band — held or merely watched — so a band left behind keeps alerting about a ticker that has stopped trading. *Values are NOT restated:* §5.1's price re-expression is SPLIT-scoped, so an EXCHANGE changes which row the position lives on and not what a share is worth. *Never overwrites* a band the owner set on the destination themselves — that is their judgement about the destination security, and no merge rule could be right. *System fit:* one predicate (`store.pending_band_move`) read by the preview and by the commit, so the sentence shown before saving cannot outlive the rule that performs it; and `target_set_at` rides across unchanged, which is why this is the one deliberate second writer of those columns — `upsert_instrument` would *derive* the stamp and re-date a band whose value never moved. ⚠ **Not reversible:** deleting the EXCHANGE does not move it back. Stated rather than engineered around — the band is a setting, not money of record, and 重算 never covered it. *Rejected:* a first-class "rename ticker" operation that rewrites the instrument row and every ledger row's symbol — it would put the ledger permanently out of step with the broker statement, which still says the old code, and the purge cascade shows signals / alerts / target weights would all have to move too |
| **D48** *(owner ruling 2026-08-15)* | **A SPINOFF's destination is a security that does not exist until the event being recorded, yet E10 hard-rejects it as unregistered — and once created it has no price, which blanks the whole portfolio's XIRR.** | **(a) Auto-register the child on save, inheriting the parent's market and quote currency; (b) let the form seed its first price.** 「spinoff則是分割出來的股份為自動註冊新股ticker盤後再更新或該行動的輸入原股份新價格」. *Inheritance is a derivation, not a guess:* **E11 already requires both ends to share a quote currency**, so any other value could not validate. Name / sector / industry stay **blank** — they are facts about a different company, and a child wearing its parent's name is worse than one wearing none. *SOFT, never silent* (implementer's reading, recorded for challenge): D19/E10 exists so a broker identifier off a statement never becomes an instrument, and silent creation would turn one mistyped box into a permanent phantom symbol no provider will price — so the owner is shown exactly what will be created and agrees. Friction is nil: this form already collects other acknowledgements. **Narrowed to SPINOFF only** — a source symbol and an EXCHANGE's destination keep E10's hard rejection. *No network call at registration,* so saving cannot fail because a provider has not listed a day-one spin-off. **(b)** matters because `returns.py` is all-or-nothing on the terminal value — ONE unpriced holding returns `rate=None` for the **whole** portfolio — and a SPINOFF is guaranteed to create one; the write goes through `pricing.store.upsert_prices` **called from `api/`**, since `pricing/` owns every price row and `data_ingestion` may not reach in. A malformed or inapplicable value is **refused**, not dropped: this field's only effect is a row nobody looks at until the XIRR stays dark |
...
| **W9** | Accounting manual | **DONE** (2026-08-11) — manual v1.7, §4.4, zh + en mirror | W3, W6b · ~~D36~~ **D45** | `docs/accounting-formula-manual.md` (zh, authority) + `.en.md` mirror | **§7.5** — including its two limitation rules: D11 stated as standing, **D12 also stated as standing** (~~resolved by D36~~ — **D45, 2026-08-11, retired D36 unimplemented**, so the reorganisation fee is invisible to every return metric this system has and the manual must not imply a resolution is coming), and cash-and-stock documented as a **hard exclusion** per D34.<br>**Corrected 2026-08-13**: this row still described the pre-D45 outcome. The manual itself was already fixed (v1.7a, both mirrors); this cell had not been propagated |
| **W6c** | Price basis — the **series** readers (**D42**, 2026-08-11) | **DONE** (2026-08-11) | W6b | `portfolio/price_basis.py` (new `series_in`, calling `price_in` per point — one owner), and the 14 `get_price_history` call sites across 10 files under `portfolio_dash/api/`: `signals_service`, `alert_inputs` (+ its `get_latest_price` leg), `digest_service`, `dividend_inbox`, `insight_service` (×5, incl. the stored `price_at_create` scalar), `routers/dashboard`, `routers/instruments` (+ its `last` leg), `routers/performance`, `routers/prompts`, `routers/symbol` | **§7.1c** — 30 tests, every re-expressed site with a PAIRED pre-fix companion, the D38 landmine over all twelve paths, and the three deliberate non-treatments pinned. **Twelve re-express, three stay raw** (two existence checks + the trend bulk load, whose consumer already applies (d) per day). One money-of-record site: the DRIP reinvest price, which `confirm()` divides into a booked share count. Raised **D44** — the owner-entered target band is not re-expressed (ruled 2026-08-15: flagged at entry, with the restated value offered as an opt-in; see §8) |
| **W10** | E2E, demo corpus, gates, ship | **CODE DONE** (2026-08-12) — demo corpus (§7.7), `verify_corporate_actions.py` (§10.5), the `verify_live.py` check, four drawer e2e, **§7.6's full enter-through-the-UI e2e** (7 flows, `tests/e2e/test_corporate_actions_flow.py`) and the **four entry-surface defects it surfaced**. Remaining is the RELEASE itself: version bump, `stamp_asset_version.py`, the `shared/whatsnew.py` CATALOG entry, the CHANGELOG version heading, the tag, and the staged demo→prod deploy | all | `tests/e2e/`, **`scripts/seed_demo.py`**, **`scripts/verify_live.py`**, **`scripts/verify_corporate_actions.py`**, `CHANGELOG.md`, `shared/whatsnew.py` | **§7.6 + §7.7 + §7.8**, then **§10.5** — which **D46 unblocked**: it runs against the simulated corpus, not the owner's statements, and already PASSes there while FAILing on a corpus one action row short. ~~needs D37's cost totals~~ |
...
| **Output** | one line per affected ticker — `symbol · 賣超 tripped? · original_total intact? · shares reconcile?` — and a final `PASS` / `FAIL — 標的 n、缺漏 m`. Nothing else, so the output can be pasted into a session without leaking amounts. Symbols are masked when they are not ticker-shaped, because an option contract carries its strike in the string |
| **Who runs it, and against what** | **Amended by D46 (2026-08-12).** The release gate runs against the **simulated corpus** (D43) — structurally faithful to the owner's export, amounts fuzzed — because D37's real cost totals are a long-lead owner item and the owner ruled they are not worth blocking on. The **real-data** run is unchanged, still available, still the owner's, and now a **post-release verification** rather than a gate |
| **What each run proves** | They are different claims and only one is now evidenced. The simulated run proves the feature **accepts the shape** — the oversell-resolved-by-a-split pattern, chained actions, multi-account sets, a SPINOFF child traded nowhere. It cannot prove the owner's **actual** cost basis reconciles; only real figures can. This row exists so the distinction is not lost the first time someone reads 「§10.5 PASS」 |
| **When** | after W10, **before** `/ship-version` — i.e. before anything is tagged, and therefore before the demo site or prod see it |
| **On failure** | **blocking.** Not a note, not a follow-up. The feature exists for this one outcome; shipping it while this fails would be shipping the thing the spec says is broken. The gate has been red — a corpus one action row short FAILs at exit 1 — so it is a gate and not a formality |
...
**D37 was the programme's longest lead item, and D46 (2026-08-12) took it off the critical path.**
Every position whose earliest event in the broker export is a **sell** needs a per-symbol
`original_cost_total` for its opening inventory; the export does not carry one, and nobody but the
owner can supply it. The owner ruled that the release does not wait for it: the gate runs on the
simulated corpus, which carries its own opening rows (including two on a pre-action share basis) and
therefore exercises this path. **The engineering half of D37 was never the blocker and is done** —
the hard validation below shipped with W7. ⚠ **The shortcut is
