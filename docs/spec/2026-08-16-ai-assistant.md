...
| **AI-D42** | 投資邏輯審查 · 現金帳進報酬 | **四類進 XIRR**：`xirr_reporting` 新增**必填**參數 `cash_movements`，只收 `REBATE`／`INTEREST`／`INTEREST_EXPENSE`／`BROKER_FEE`（**池內損益**），符號取自既有 `cash_kinds.CASH_KIND_TABLE.credit`，每筆按自身日期 FX 換算；`DEPOSIT`／`WITHDRAW`／`OPENING`（**資本移動**）不進 | owner 裁納 2026-08-24（審查 §2.2）。`xirr_reporting` 現行簽名**根本沒有** `cash_movements` 參數，所以 FE-D1 的 77% 退佣（群益 charge-first，單趟差 **0.229% 資本**，且那是券商常態計費）與所有利息／券商費永遠進不了任何報酬指標。**這條線 `cash_kinds.py` 已經畫好了**——該模組的兩軸表（`credit` × `fx_acquisition`）分的正是「資本移動 vs 池內損益」，重用它即可，零新詞彙。⚠ 納入資本移動會把 XIRR 從「投入證券的資金報酬」變成「帳戶報酬」，是另一種指標，**不在此裁示內**。否決：只收 REBATE（利息與券商費仍是黑洞）、不改 XIRR 只加揭露列（指標仍不完整） |
