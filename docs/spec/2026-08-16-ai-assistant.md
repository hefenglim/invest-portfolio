...
| **AI-D60** | R4 的缺腿 · 基準對照進提示詞 | ★ **補上 `benchmark_counterfactual_json`（36 → 37），並讓官方模板真的引用它。** R4 出貨時把反事實接上了儀表板與抽屜，卻沒有做計畫 §8.3 明文要求的提示詞變數——所以錨裝好了，**敘事的那個人背對著它**：沒有任何一張洞察卡能說「你贏了／輸給指數」，而那正是 R4 被四鏡頭審查點名為「最高價值的一件事」的理由。⚠ **落點是持倉週報（組合層），不是個股建議卡**：反事實是**組合層**數字，放進逐標的卡等於邀請模型寫出「這檔股票贏了大盤」——`kpis_json` 的 `scope_note` 已經為同一個陷阱留過註解（「a per_market card must not read them as this market's numbers」）。⚠ **涵蓋率紅線做進資料本身**：`uncovered_ratio > 0` 時變數自帶 `coverage_note`，模板明令照抄且禁用「超額報酬／打敗大盤」措辭——不指望提示詞記得，因為 `BenchmarkComparison` 的 docstring 早就禁止在該狀態印出裸的「超額報酬」 | owner 裁示 2026-08-28（選「補變數＋模板引用」）。週報 v2.4 → v2.5、`LIBRARY_VERSION` → `official-v20`；三支變數計數測試同步 36 → 37，`test_variables.py` 的類別計數 position 6 → 7 |
| **AI-D61** | `is_etf` · 在入口就問，而不是等賣出才提示 | ★ **手動交易的自動註冊路徑可以攜帶 ETF 答案**（`ManualBody.new_symbol_is_etf`），前端在「未註冊」提示旁長出勾選框。owner 提案 2026-08-28：與其「註冊（未知）→ 交易 → 賣出時跳軟性 issue → 去標的管理補勾 → 回頭確認稅率」，不如在使用者本來就在看那個代號的那一刻問一次。⚠ **只在 TW／MY 出現**：那是旗標會改變金額的兩個市場（TW 賣出證交稅 0.3% vs ETF 0.1%；MY 印花稅 ETF 全免），美股放這個框等於一個永遠不做事的欄位。⚠ **命名為 `new_symbol_is_etf` 而非 `is_etf`，差別就是全部**：它只在**自動註冊那一行**被讀取，已註冊的標的永遠到不了那裡——2026-07-15 壓力稽核修掉的正是「輸入旗標勝過註冊表」，這個欄位絕不能把它放回來 | owner 提案並裁納 2026-08-28。AI-D40 未被放寬：不答仍記未知，首次 TW 賣出仍發 `etf_flag_unknown`。★ 動機是實測的：2026-08-28 對 demo 的唯讀稽核抓到 `0056` 元大高股息以 `is_etf=False` 註冊、0 筆賣出——**還沒錯稅，第一次賣出才會錯**。`quick_register` 本來就接受`bool | None` 並翻成 `(is_etf, etf_flag_unknown)` 那一對，所以缺的只有 body 欄位與前端 |
| **AI-D62** | W8 下半 · 週線視角（唯讀鏡頭） | ★ **只做讀取時的重採樣＋一個 per-symbol 變數`weekly_signals_json`（37 → 38），不碰任何會持久化的東西。** 明確不動：`signal_history` 的主鍵仍是 `(symbol, as_of)`、TechScore、`alert_events`、事件研究回測、`rules-v1` 參數集。⚠ 這個切分不是圖省事，而是**主鍵少一維會讓日線與週線互相覆寫**，而混在同一個鍵下的 TechScore 會把兩個不同的問題平均起來卻沒人看得出來——真正的第二時間框要先加 timeframe 維度並遷移既有資料，那是W6 等級的波次。⚠ 每個數字以「週」命名（`ma10w`）、payload 自帶 `timeframe`，模板明令兩個時間框**不可互相比較、不可混寫在同一句**——AI-D2 的「同名兩定義」教訓提前套用 | owner 裁示 2026-08-28（在「唯讀鏡頭 / 完整第二時間框 / 不做」三選一中選唯讀鏡頭）。重採樣按 **ISO 週**分組取當週最後一個收盤，**不是每 5 列切一次**：一個四天的假日週會把下週一推進上一週，而誤差隨全年假日累積而不是抵銷。已接進個股健檢卡（v2.8 → v2.9，`LIBRARY_VERSION` → `official-v21`）——W6 已經示範過「變數上線但零模板引用」會暗一整波 |
| **AI-D63** | W4 · 抽取準確度的 live 基線（13 次跑） | ★ **基線：欄位級 99.3–100%（最佳一次 295/295），三個「會動錢卻不報錯」的欄位全乾淨——cash kind 12/12、short_sale 24/24、daytrade 24/24。** 模型 `google/gemini-2.5-flash`（OpenRouter），owner 於 2026-08-28 提供專用金鑰。**已知偶發（非確定性，逐一列出而非藏進平均）**：`div-tw-stock` ≈29%（配股的 gross 被讀成股數 50；當時記為「重放不讀該欄，影響限於股利帳面」——⚠ **該句已由 AI-D64 更正：當時無法證實，且若模型同時漏填 `reinvest_shares`，重放會整本拋錯**。原文保留備查）、`english-statement-fragment` ≈23%（英文片段的日期解析回退到 today）、`unparsed-ambiguous-cash` ≈23%（該進 unparsed 的列被猜成資金列）。⚠ 門檻**維持 OFF**：手動傳入的建議值是 `--max-cash-kind-miss 0 --max-daytrade-miss 0 --max-short-miss 0`，但 `--min-field-hit` **不設**——這個 runner 非確定性，把單次跑釘成預設門檻會讓下一次的雜訊看起來像回歸 | ★ **三件事都是多次跑才看得出來的，單次跑一件也看不到。** **(1) 治具缺陷偽裝成模型失敗**：首跑 34/40，六個失敗裡四個是 `RUNNER ERROR: no such table: fx_rates`——評測器只建帳本表、沒建 `pricing` 的表，所以**三個馬股案例＋合併帳戶美股案例根本沒被評到**。馬股準確度不是低，是未量測；修好後可評欄位 263 → 295。 **(2) daytrade 是真缺陷，散文擋不住**：提示詞早已明文 `never infer it from two same-day opposite drafts`，模型仍在使用者未明示當沖的當日來回自設 `daytrade=1`（會把台股賣出證交稅 0.3% 砍成 0.15%）。加**負面 one-shot** 後 **0/2 → 11/11**。 **(3) ★ 對 `div-tw-stock` 的「顯而易見的修法」被量測證明是壞交易**：同樣加一個 STOCK one-shot，該案例 6/6 通過——但它教會模型把**其他股利類型的 gross 也歸零**，`div-my-net` 從 **0/4 失敗變成 4/6 失敗**（真實金額 88.50 被寫成 0）。用「配股 gross 印成 50（重放不讀）」換「馬股淨額歸零」，是嚴格更差。**one-shot 已整個回退**，`div-tw-stock` 記錄為已知限制而非假裝修好。這正是語料存在的理由：它讓「這個修法其實是回歸」變成看得見的事實 | owner 提供金鑰 2026-08-28；提示詞 v7 → v7.1（僅 daytrade one-shot），`LIBRARY_VERSION` → `official-v22` |
| **AI-D64** | LLM 失敗情境的留存 | ★ **SQLite 表 `llm_fail_log` ＋ `.jsonl` 匯出**（owner 裁示 2026-08-28）。全系統原本**沒有任何地方**保留提示詞或模型原始回覆：`shared/llm.py` 把 `messages` 與 `content` 都當區域變數，解析完即丟——成功如此，失敗也如此；`llm_usage` 只存 token 與成本。所以抽取出錯時，唯一倖存的證據是「哪個欄位不符」。⚠ **這不是假想**：AI-D63 把配股失敗記成「重放不讀 `gross`，影響限於股利帳面」，而該結論**當時就無法查證**。本輪查明匯入端**沒有守衛**要求 STOCK 帶 `reinvest_shares`（`dividend_import.py` 原樣傳遞、`dividend_model.py` 不計算、守恆閘 `check_amounts(gross,0,0)` 對任何非負 gross 都通過），而重放端**會拋錯**（`cost_basis.py:606-613`）——`test_cost_basis.py:234` 記著這一類曾「crashed every rebuild that held a MY dividend」。那列究竟無害還是地雷，取決於報表從未捕捉的一個欄位 | **逐次嘗試一列，不是逐次呼叫**：seam 先扣款再解析、且會 salvage 後再打一次，一個邏輯失敗＝兩次計費＋兩份原始輸出；每列帶自己那次的 `llm_usage.id` 以對帳。**`agent` 就是分類**（每個呼叫點本來就傳，且已進 `llm_usage`）——生產端無法得知更細的類別，失敗當下還不知道那是股利。⚠ **紅線以測試釘死而非註解**：清除本表**絕不得**碰 `llm_usage`，因為剩餘額度＝`Σ加值 − Σ用量`，刪用量列等於白送額度。⚠ 選 SQLite 而非散落 `.jsonl` 檔：`.gitignore` 目前**不**忽略 `*.jsonl`，而 `data/` 已整個忽略；且備份／db-stats／清除全部沿用現成機制 |
| **AI-D65** | 記錄範圍 | ★ **硬失敗 ＋ AI 門的 `unparsed` 坦白**（owner 裁示 2026-08-28）。硬失敗＝完全沒拿到可用結果（供應商錯誤／JSON 壞掉／schema 不符／額度用盡／未啟用）。`unparsed`＝**成功**的呼叫，但模型自己承認某幾列無法歸類——那些列目前顯示在畫面上然後消失 | 後者是微調語料裡最有價值的部分：它是模型**自陳的**能力邊界，且已經帶著原文。⚠ 未納入 needs-confirm 的軟性議題：那多半不是 LLM 的錯（例如股利類型與帳戶模型不符），會稀釋語料 |
| **AI-D66** | 保留策略 | ★ **上限環形，保留最新 300 筆**，寫入時順便修剪（`api/action_log.py` 的 `_KEEP` 前例）；每個文字欄上限 16 KiB 並標 `truncated` | owner 同時要「維持系統空間健康」與「作為微調資料集」，兩者相反，故取**硬上限＋可見的滾動**：面板顯示「N/300 筆・最舊 YYYY-MM-DD」，那就是該打包下載的提示。最壞情況 ≈ 300 × 3 × 16 KiB ≈ 14 MiB，是天花板不是期望值。⚠ **base64 影像絕不入庫**（一次視覺呼叫可帶 4 張各 5 MB）：只留文字段落與 `image_count` |
| **AI-D67** | 順序：先捕捉、後實驗 | ★ **捕捉縫先做，提示詞實驗後跑**（owner 裁示 2026-08-28）。評測器同步取得 `--dump-dir`：每次嘗試一行 `.jsonl`，帶完整提示詞、模型逐字回覆、案例 id、判定與欄位級差異 | 理由是 AI-D63 本身：那一輪的結論建立在只有「want X got Y」的證據上。⚠ **評測模式必須連成功的嘗試也捕捉**——以欄位比對「失敗」的案例其實回了**可解析**的 JSON，生產策略下不會留下任何記錄，而那正是最需要讀原始回覆的案例。以顯式 `set_capture_mode()` 開關控制，生產預設**關** |
| **AI-D68** | 提示詞的 one-shot **不得**是語料案例逐字 | ★ **新守衛 `tests/llm_insight/test_prompt_examples_are_not_the_corpus.py`**，任何 `<example_input>` 與語料 `input` 逐字相同即建置失敗（含一條自我檢測：守衛本身必須看得見重複，否則它會因為「什麼都沒找到」而綠）。2026-08-28 量測時發現：W4 加的 `daytrade` 負面 one-shot **就是**語料案例 `tw-sameday-roundtrip-NOT-daytrade` 一字不差，而被回退的 STOCK one-shot **就是** `div-tw-stock` 的 input。⚠ 所以 AI-D63 記的「daytrade 0/2 → 11/11」與「STOCK one-shot 讓該案例 6/6」**量到的是背答案，不是抽取能力**——提示詞裡印著考題的答案 | 這個失效模式會**自我增強**：修一個失敗案例最直覺的做法就是把它貼進提示詞當範例，貼完該案例就過，而模型什麼也沒學到，語料於是回報一個不存在的修復——正是語料被造出來要消滅的盲目。範例仍是對的工具，只是必須換成**同形狀的另一個實例**：`daytrade` 例子改寫為 2412／不同日期價格，股利對照組用 2891／5347、不同日期金額，四個範例的符號日期金額**全部不在語料中** |
| **AI-D69** | 外溢的成因是**內容**，不是範例的形式 | ★ **推翻 AI-D63 的診斷，以 11 次跑量測。** AI-D63 把 `div-my-net` 的回歸歸咎於 one-shot 的形狀（「模型看到範例裡的 `gross:"0"` 就過度泛化」）。臂 B **完全沒有任何範例**，只把規則改寫成結構化散文＋`[Exception]` 宣告，**同一個回歸原樣重現**：11 次跑裡有 8 次把真實的 RM88.50 淨額股利寫成 `gross:"0"`（`div-my-net` 11/11 → **3/11**）。結論：**任何**說「STOCK 的 gross 是 0」的指示都會跨股利類型外溢，與它以範例或散文交付無關 | AI-D63 自陳「四種配置只量了三種，prose-only 從未單獨隔離」——這一輪把它量了，而它失敗。⚠ 附帶更正：AI-D63 的 `div-tw-stock ≈29%` **從來不是有效對照**，本輪同工作階段重測為 **10/11 = 91%**；29% 是把已回退的 one-shot 實驗混在一起算的。因此那個回歸換來的「6/6 修復」很可能根本不存在，而它造成的 `div-my-net` 回歸是大且真實的——**回退是對的，但理由比當初記的更好**。AI-D63 原文保留並標註 |
| **AI-D70** | 對照組成對（owner 技巧 ②）＋ 結構化條件（技巧 ①） | ★ **提示詞 v7.1 → v7.3、`LIBRARY_VERSION` → `official-v23`；B2 臂 11 次跑全部 295/295 欄位命中，案例層 423/440 → 434/440。** 技巧 ①：欄位規則綁進型別自己的定義（`STOCK -> [Rule]: MUST set gross="0"，股數必須進 reinvest_shares`），並用 `[Exception]` 明文打破「沒寫就不填 vs gross 是必填欄」的規格矛盾。技巧 ②：**絕不單獨給一個極端邊界**——STOCK（gross=0）與 MY NET（gross=42.75）**成對出現**，讓模型看出 `gross` 隨 `type` 變化。成對範例把 ① 造成的回歸**完整修復**：`div-my-net` 3/11 → **11/11** | ★ **最大的贏面不在目標上**：`english-statement-fragment`（一個行首日期管三個項目，模型只對第一項用對日期，股利與券商費用都退回 today——**兩個都是記錄性數字**）**2/11 → 11/11**。臂 B 在完全沒有日期規則的情況下已經把它推到 8/11，顯示**把跑馬燈式長段落切成標籤區塊，連帶改善了鄰近規則的遵從度**。因此原訂的日期範圍規則**不加**：缺陷已不重現，為量不到的東西加規則是沒有證據的複雜度。⚠ 一個回歸保留並揭露：`unparsed-option` 11/11 → 7/11——模型仍拒收、仍坦白，只是引文掉了 ticker（`unparsed_contains: ["TSLA"]` 的子字串檢查因此失敗），**不偽造任何列**；知情換取其他 +15，含兩個記錄性日期。⚠ 誠實界限：對照組與 `div-tw-stock`／`div-my-net` 同句型（換符號日期金額），這兩案例因此部分在量樣式辨識；獨立證據是其餘 38 案，尤其提示詞中無對應範例的 `english-statement-fragment` |
...
| **W8** | OHLC `*_raw`＋factor 地基（AI-D58）· 週線視角＝唯讀鏡頭（AI-D62） | 2026-08-28 | M | ✅ 兩半皆出貨 |
