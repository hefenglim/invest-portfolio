...
| F-44 | L | `spec:900-905`, `:1832` | `dashboard.py` anchors have drifted ~28 lines (`price_map` is `:236`, `xirr_reporting` `:296`) and the spec instructs editing by line number **(A)** | Anchor on the construct, not the number |
...
| F-46 | L | `backlog:135` | The backlog names `export/tax.py` as an inline `quantity × price` site; it has none — it consumes pre-computed `RealizedRow` fields. The real set is **12 files, not 5**, making P3's blocker larger than recorded **(A)** | Correct the backlog's site list |
...
- **P0 does not make P3 (options) harder (A).** Inline `quantity × price` sites: **24 at merge-base
  `734833b`, 24 at `f80e462`** — per-file counts identical, only line numbers moved. `apply_ratio`
  operates on a quantity and never forms money; `split_factor` is prices-only; `_apply_action`
  *transfers* totals rather than re-deriving them from `qty × price`. W4 and W6 add none either.
