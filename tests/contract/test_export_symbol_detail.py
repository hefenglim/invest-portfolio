...
def test_export_symbol_detail_non_ascii_symbol_never_500(
    ...
    """A registered non-ASCII symbol must not crash header encoding (never a 500). The
    Content-Disposition stays latin-1 safe and carries the full name via filename*."""
    upsert_instrument(golden_db, Instrument(symbol="台積電", market=Market.TW,
                                            quote_ccy=Currency.TWD, sector="Semi",
                                            name="TSMC", board="TWSE"))
    golden_db.commit()
    r = api_client.post("/api/export/symbol-detail", json={"symbol": "台積電"})
    assert r.status_code == 200
    cd = r.headers["content-disposition"]
    cd.encode("latin-1")  # header value must be pure latin-1 (no crash)
