...
Asserts the contract the owner ruled on:
  ...
  * the cash row shows the server-owned 中文 kind label AND an explicit sign (＋入金) — a
    mislabelled kind reverses the pool by 2× with no error, so the label+sign ARE the guard;
...
def test_mixed_union_renders_three_sections_and_commits_per_kind(
    ...
    cash_kind = page.locator("#ai-body-cash td:nth-child(4) .dir-chip")
    expect(cash_kind).to_have_text("＋ 入金")
    expect(page.locator("#ai-body-cash td:nth-child(6)")).to_contain_text("＋")
