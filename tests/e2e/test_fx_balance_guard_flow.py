...
Scenario: moomoo_my_us starts with a clean MYR 50,000 pool (no golden flows touch it).
...
def _seed_fx(conn: Any) -> None:
    """Golden scenario + a clean, KNOWN moomoo_my_us MYR pool (funding ccy), so the balance
    ceiling is unambiguous (no golden flow touches moomoo_my_us)."""
    _seed_golden(conn)
...
def test_fx_center_balance_line_and_hard_oversell_block(
    ...
    assert _cash_balance(base, "moomoo_my_us", "USD") == "10000"
