...
def test_opening_inventory_seeds_position() -> None:
    ...
    book = build_book(LedgerBundle(opening=[oi], instruments=INSTR))
    h = book.holdings[0]
...
def test_dividend_for_unknown_position_raises() -> None:
    ...
    with pytest.raises(ValueError, match="unknown position"):
        build_book(LedgerBundle(dividends=divs, instruments=INSTR))
...
def test_opening_inventory_plus_subsequent_buy_accumulates() -> None:
    ...
    book = build_book(LedgerBundle(txs, opening=[oi], instruments=INSTR))
    h = book.holdings[0]
    assert h.shares == Decimal("20")
...
def test_drip_without_reinvest_shares_raises() -> None:
    ...
    with pytest.raises(ValueError, match="requires reinvest_shares"):
        build_book(LedgerBundle(txs, divs, instruments=INSTR))
...
def test_realized_row_carries_sell_date() -> None:
    ...
    book = build_book(LedgerBundle(txs, instruments=inst))
...
def test_net_dividend_reduces_adjusted_like_cash() -> None:
    ...
    book = build_book(LedgerBundle(txs, divs, instruments=INSTR))  # must not raise
    h = book.holdings[0]
